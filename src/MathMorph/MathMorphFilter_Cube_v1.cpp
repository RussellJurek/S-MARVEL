...
// c++ MathMorphFilter_Cube_v1.cpp -o MathMorphFilter_Cube_v1 -O2 -lcfitsio
...
int main(int argc, char* argv[]){
  ...
  // open output file
  status = 0;
  outputfile_name = outputfile_code + "_open.fits";
  ...
  status = 0;
  outputfile_name = outputfile_code + "_diff.fits";
  cout << "Attempting to create output file: " << outputfile_name << " using " << inputfile_name << " as a template." << endl;
  outputfile_name = "!" + outputfile_name;
  ...
  for(cy = 0; cy < NOcy; cy++){
    ...
    for(cx = 0; cx < NOcx; cx++){
      ...
      MathMorphOpen(data_vals,(fits_read_finish[0] - fits_read_start[0] + 1),(fits_read_finish[1] - fits_read_start[1] + 1),(fits_read_finish[2] - fits_read_start[2] + 1),sradius,fradius,round_flag);
