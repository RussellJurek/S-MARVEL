...
void NonLinFilter_2D(data_type * image, int NOx, int NOy, bool elliptical, int r_x, int r_y, unsigned short int filter_mode, unsigned short int vb_flag) {
  ...
  size_t * test_region, max_val_index, * map_p_to_t, buffer_write, buffer_read;
  ...
  for(j = 1 + r_y; j < NOy; ++j){
    ...
    buffer[buffer_write*NOx] = filter(image,test_region,NOt);
	...
	// for(os = 0; os < NOos; ++os){
      ...
      buffer[(buffer_write*NOx) + i] = filter(image,test_region,NOt);
