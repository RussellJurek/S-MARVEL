...
void MinSmooth(data_type * data_vals, int size_x, int size_y, int size_z, int sradius, int fradius, int axis, int round_flag, int vb_flag){
  ...
  data_type * data_orig;
  int y_min,y_max,z_min,z_max,tid,NOt;
  
  // retrieve the number of threads available and specify this many to be used
  NOt = omp_get_max_threads();
  omp_set_num_threads(NOt);
  
  // allocate space for a copy of the input array as data_orig, that gets read from by the parallel threads instead of the original, modified data
  data_orig = new data_type[(size_x * size_y * size_z)];
  
  // start of parallel region
#pragma omp parallel default(shared) private(i,j,k,s,bdepth,used,NOvals,tid,NOt,y_min,y_max,z_min,z_max,buffer_vals,temp_vals,sep,sep_max,progress) 
  { 
    
    // retrive the thread id
    tid = omp_get_thread_num();
    
    // calculate the region to be processed by this thread
    if(size_z <= 1){
      
      y_min = tid * static_cast<int>(ceil(static_cast<double>(size_y) / static_cast<double>(NOt)));
      y_max = (tid + 1) * static_cast<int>(ceil(static_cast<double>(size_y) / static_cast<double>(NOt)));
      ...
      y_min = 0;
      y_max = size_y;
      z_min = tid * static_cast<int>(ceil(static_cast<double>(size_z) / static_cast<double>(NOt)));
      z_max = (tid + 1) * static_cast<int>(ceil(static_cast<double>(size_z) / static_cast<double>(NOt)));
      
    }
    
    // copy the data_vals array into data_orig
	...
	for(i = 0; i < size_x; ++i){
	  ...
	  data_orig[((k * size_x * size_y) + (j * size_x) + i)] = data_vals[((k * size_x * size_y) + (j * size_x) + i)];
...
#pragma omp single 
	...
	for(k = z_min; (k < size_z) && (k < z_max); ++k){
	  
	  for(j = y_min; (j < size_y) && (j < y_max); ++j){
	    
	    // populate array of temp values
	    used = Populate_temp_array(temp_vals,data_orig,i,j,k,size_x,size_y,size_z,sradius,fradius,round_flag);
	    
	    // calculate minimum of temp values and update buffer_vals
	    buffer_vals[((k*bdepth*size_y) + (j*bdepth) + i)] = Temp_array_min(temp_vals,data_orig,i,j,k,size_x,size_y,size_z,used); 
...
#pragma omp single 
	...
	for(k = z_min; (k < size_z) && (k < z_max); ++k){
	  ...
	  for(j = y_min; (j < size_y) && (j < y_max); ++j){
	    ...
	    // calculate minimum of temp values and update buffer_vals
	    buffer_vals[((k*bdepth*size_y) + (j*bdepth) + bdepth - 1)] = Temp_array_min(temp_vals,data_orig,i,j,k,size_x,size_y,size_z,used);
...
#pragma omp master
	  {
	    while(progress <= (((float) (i + 1)) / ((float) size_x))){ cout << "*"; cout.flush(); progress+=(0.05*NOt); } 
	  }
...
#pragma omp master
    ...
    case 2:
      ...
      // fill initial buffer values	
      for(j = y_min; (j < (bdepth - 1)) && (j < y_max); ++j){
	...
	for(k = z_min; (k < size_z) && (k < z_max); ++k){
	  ...
	  for(i = 0; i < size_x; ++i){
	    ...
	    // calculate minimum of temp values and update buffer_vals
	    buffer_vals[((k*bdepth*size_x) + (j*size_x) + i)] = Temp_array_min(temp_vals,data_orig,i,j,k,size_x,size_y,size_z,used);
...
#pragma omp single
	{
      ...
      progress = 0.0;
      for(j = y_min + (bdepth - 1); (j < size_y) && (j < y_max); ++j){
	...
	for(k = z_min; (k < size_z) && (k < z_max); ++k){
	  ...
	  for(i = 0; i < size_x; ++i){
	    ...
	    // calculate minimum of temp values and update buffer_vals
	    buffer_vals[((k*bdepth*size_x) + ((bdepth - 1)*size_x) + i)] = Temp_array_min(temp_vals,data_orig,i,j,k,size_x,size_y,size_z,used);
...
#pragma omp master
	  {
	    while(progress <= (((float) (j + 1)) / ((float) size_y))){ cout << "*"; cout.flush(); progress+=(0.05*NOt); } 
	  }
    ...
    case 3:
      ...
      // fill initial buffer values
      for(k = z_min; (k < (bdepth - 1)) && (k < z_max); ++k){
	...
	for(i = 0; i < size_x; ++i){
	  ...
	  for(j = y_min; (j < size_y) && (j < y_max); ++j){
	    ...
	    // calculate minimum of temp values and update buffer_vals
	    buffer_vals[((k*size_x*size_y) + (j*size_x) + i)] = Temp_array_min(temp_vals,data_orig,i,j,k,size_x,size_y,size_z,used);
...
#pragma omp single
	{
      ...
      progress = 0.0;
      for(k = z_min + (bdepth - 1); (k < size_z) && (k < z_max); ++k){
	...
	for(i = 0; i < size_x; ++i){
	  ...
	  for(j = y_min; (j < size_y) && (j < y_max); ++j){
	    ...
	    // calculate minimum of temp values and update buffer_vals
	    buffer_vals[(((bdepth - 1)*size_x*size_y) + (j*size_x) + i)] = Temp_array_min(temp_vals,data_orig,i,j,k,size_x,size_y,size_z,used);
	...
	if(vb_flag > 0){ 
	  ...
	  #pragma omp master
	  {
	    while(progress <= (((float) (k + 1)) / ((float) size_z))){ cout << "*"; cout.flush(); progress+=(0.05*NOt); } }
	  }
...
#pragma omp single 
	...
	for(k = z_min; (k < size_z) && (k < z_max); ++k){
	  ...
	  for(j = y_min; (j < size_y) && (j < y_max); ++j){
	    ...
	    // calculate minimum of temp values and update buffer_vals
	    buffer_vals[((k*bdepth*size_y) + (j*bdepth) + i)] = Temp_array_max(temp_vals,data_orig,i,j,k,size_x,size_y,size_z,used); 
...
#pragma omp single 
	...
	for(k = z_min; (k < size_z) && (k < z_max); ++k){
	  ...
	  for(j = y_min; (j < size_y) && (j < y_max); ++j){
	    ...
	    // calculate minimum of temp values and update buffer_vals
	    buffer_vals[((k*bdepth*size_y) + (j*bdepth) + bdepth - 1)] = Temp_array_max(temp_vals,data_orig,i,j,k,size_x,size_y,size_z,used);
...
#pragma omp master
	...
	for(k = z_min; (k < size_z) && (k < z_max); ++k){
	  ...
	  for(i = 0; i < size_x; ++i){
	    ...
	    // calculate minimum of temp values and update buffer_vals
	    buffer_vals[((k*bdepth*size_x) + (j*size_x) + i)] = Temp_array_max(temp_vals,data_orig,i,j,k,size_x,size_y,size_z,used);
...
#pragma omp single
	...
	for(k = z_min; (k < size_z) && (k < z_max); ++k){
	  ...
	  for(i = 0; i < size_x; ++i){
	    ...
	    // calculate minimum of temp values and update buffer_vals
	    buffer_vals[((k*bdepth*size_x) + ((bdepth - 1)*size_x) + i)] = Temp_array_max(temp_vals,data_orig,i,j,k,size_x,size_y,size_z,used);
...
#pragma omp master
	...
	for(i = 0; i < size_x; ++i){
	  ...
	  for(j = y_min; (j < size_y) && (j < y_max); ++j){
	    ...
	    // calculate minimum of temp values and update buffer_vals
	    buffer_vals[((k*size_x*size_y) + (j*size_x) + i)] = Temp_array_max(temp_vals,data_orig,i,j,k,size_x,size_y,size_z,used);
...
#pragma omp single
	...
	for(i = 0; i < size_x; ++i){
	  ...
	  for(j = y_min; (j < size_y) && (j < y_max); ++j){
	    ...
	    // calculate minimum of temp values and update buffer_vals
	    buffer_vals[(((bdepth - 1)*size_x*size_y) + (j*size_x) + i)] = Temp_array_max(temp_vals,data_orig,i,j,k,size_x,size_y,size_z,used);
...
void MedianSmooth(data_type * data_vals, int size_x, int size_y, int size_z, int sradius, int fradius, int axis, int round_flag, int vb_flag){
  ...
  // start of parallel region
#pragma omp parallel default(shared) private(i,j,k,s,bdepth,used,NOvals,tid,y_min,y_max,z_min,z_max,buffer_vals,temp_vals,sep,sep_max,progress) 
  { 
...
#pragma omp single 
	...
	for(k = z_min; (k < size_z) && (k < z_max); ++k){
	  ...
	  for(j = y_min; (j < size_y) && (j < y_max); ++j){
	    ...
	    // calculate minimum of temp values and update buffer_vals
	    buffer_vals[((k*bdepth*size_y) + (j*bdepth) + i)] = Temp_array_median(temp_vals,data_orig,i,j,k,size_x,size_y,size_z,used); 
...
#pragma omp single 
	...
	for(k = z_min; (k < size_z) && (k < z_max); ++k){
	  ...
	  for(j = y_min; (j < size_y) && (j < y_max); ++j){
	    ...
	    // calculate minimum of temp values and update buffer_vals
	    buffer_vals[((k*bdepth*size_y) + (j*bdepth) + bdepth - 1)] = Temp_array_median(temp_vals,data_orig,i,j,k,size_x,size_y,size_z,used);
...
#pragma omp master
	...
	for(k = z_min; (k < size_z) && (k < z_max); ++k){
	  ...
	  for(i = 0; i < size_x; ++i){
	    ...
	    // calculate minimum of temp values and update buffer_vals
	    buffer_vals[((k*bdepth*size_x) + (j*size_x) + i)] = Temp_array_median(temp_vals,data_orig,i,j,k,size_x,size_y,size_z,used);
...
#pragma omp single
	...
	for(k = z_min; (k < size_z) && (k < z_max); ++k){
	  ...
	  for(i = 0; i < size_x; ++i){
	    ...
	    // calculate minimum of temp values and update buffer_vals
	    buffer_vals[((k*bdepth*size_x) + ((bdepth - 1)*size_x) + i)] = Temp_array_median(temp_vals,data_orig,i,j,k,size_x,size_y,size_z,used);
...
#pragma omp master
	  {
	    while(progress <= (((float) (j + 1 - y_min)) / ((float) (y_max - y_min)))){ cout << "*"; cout.flush(); progress+=0.05; } 
	  }
	...
	for(i = 0; i < size_x; ++i){
	  ...
	  for(j = y_min; (j < size_y) && (j < y_max); ++j){
	    ...
	    // calculate minimum of temp values and update buffer_vals
	    buffer_vals[((k*size_x*size_y) + (j*size_x) + i)] = Temp_array_median(temp_vals,data_orig,i,j,k,size_x,size_y,size_z,used);
...
#pragma omp single
	...
	for(i = 0; i < size_x; ++i){
	  ...
	  for(j = y_min; (j < size_y) && (j < y_max); ++j){
	    ...
	    // calculate minimum of temp values and update buffer_vals
	    buffer_vals[(((bdepth - 1)*size_x*size_y) + (j*size_x) + i)] = Temp_array_median(temp_vals,data_orig,i,j,k,size_x,size_y,size_z,used);
	...
	if(vb_flag > 0){ 
	  ...
	  #pragma omp master
	  {
	    while(progress <= (((float) (k + 1 - z_min)) / ((float) (z_max - z_min)))){ cout << "*"; cout.flush(); progress+=0.05; } }
	  }
...
void HeapSort_index(int n, T_data ra[], int indices[]){
